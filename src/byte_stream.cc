...
{
  bytes_to_push = min(
    data.length(), available_capacity() ); // using min to ensure that data pushed does not exceed avail capacity
...
uint64_t Writer::available_capacity() const
{
  return capacity_
         - buffer
             .length(); // return the avaialble amount by subtracting the total capacity with current buffer size
}
...
{
  return buffer; // the string view library enables us to see the contents without having to retrieve the contents
                 // as a temp window
}

void Reader::pop( uint64_t len )
{
  bytes_to_pop = min( len, bytes_buffered() ); // uses similar initial logic as push, it ensures that kay sirf jitni
                                               // bytes hein wo delete houn minus mein na jaye
...
bool Reader::is_finished() const
{
  return (
    writer_closed
    && buffer.empty() ); // check if writer closed is true and the buffer is empty (nothing to read basically)
}
